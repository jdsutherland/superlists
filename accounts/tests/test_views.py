...
class SendingLoginEmailViewTest(TestCase):
    def test_redirects_to_homepage(self):
        response = self.client.post(
            '/accounts/send_login_email', data={'email': 'alice@example.com'})
    ...
    def test_adds_success_message(self):
        ...
        self.assertEqual(
            message.message,
            "Check your email, we've sent a link you can use to log in.")
...
class LoginViewTest(TestCase):
    ...
    def test_calls_auth_login_with_user_if_user_authenticated(self, mock_auth):
        response = self.client.get('/accounts/login?token=abcd123')
        self.assertEqual(
